...
28. **Mapa del panel Admin (v2.10.11):** abrir Home → Admin (`LSMQC$`). La pestaña **Mapa** debe dibujarse de inmediato, con fondo Esri (sin la marca "API KEY REQUIRED") y los puntos QC/LSM. El desplegable debe listar los mapas del dispositivo o decir "No hay mapas guardados en este dispositivo". Cambiar a **Tabla** y volver a **Mapa**: los tiles deben seguir viéndose (no gris). Seleccionar un mapa y pulsar **Cargar**: el overlay se dibuja y el contador dice "| Mapa cargado"; con un PDF sin georreferenciación debe salir el error y **no** el mensaje "Mapa cargado". Cerrar y reabrir el panel varias veces sin recargar la página: el mapa debe reaparecer siempre.
...
### v2.10.11 — Fix: el mapa del panel Admin no cargaba (y mapa base sin API key)

- **Problema:** en el panel de Administración la pestaña **Mapa** mostraba un recuadro gris y el desplegable de mapas locales aparecía vacío, aunque la pestaña **Tabla** sí traía los registros de Supabase. Tres defectos se sumaban:
  1. `openPanel()` sólo pintaba el mapa **dentro del `.then()` de `loadMarkers()`**: si Supabase tardaba o no respondía (los `fetch` no tenían timeout), `renderMapTab()` no llegaba a ejecutarse nunca.
  2. En `renderMapTab()`, `L.map(container)` se llamaba **antes** de `renderMapSelector()` y sin `try/catch`. Cualquier excepción de Leaflet (típicamente *"Map container is already initialized"* cuando un mapa anterior no se destruyó bien) abortaba el resto de la función, dejando el recuadro vacío **y** el selector sin opciones. Al ejecutarse dentro de un `.then()`, el error sólo aparecía en consola.
  3. Nunca se llamaba `invalidateSize()`: el mapa vive dentro de un modal, así que al medirse con tamaño 0 (modal recién mostrado o pestaña que estaba oculta) Leaflet no pedía tiles y quedaba gris.
  Además, **CARTO dejó de servir sus basemaps sin API key**: los tiles seguían llegando con HTTP 200 pero estampados con *"API KEY REQUIRED"*, tanto en el admin como en el mapa principal.
- **Solución:**
  - `openPanel()` llama `renderMapTab()` de inmediato; los marcadores se repintan cuando llegan los datos. El mapa ya no depende de la red.
  - `renderMapTab()`: el selector se renderiza primero y por separado, la creación del mapa va en `try/catch` con toast del error real, se limpia `container._leaflet_id` si quedó sucio y se llama al nuevo `refreshAdminMapSize()` (`invalidateSize` en `requestAnimationFrame` + a los 250 ms).
  - `renderMapSelector()` deja de tragarse los errores: muestra *"No hay mapas guardados en este dispositivo"* o *"Error al leer los mapas locales"* + toast, y conserva la selección previa.
  - `fetchTable()` con `AbortController` y timeout de 15 s.
  - `loadAdminMap()` reporta `e.message` en el toast; `loadAdminPDF()` **lanza** cuando el PDF no tiene georreferenciación (antes retornaba y el flujo anunciaba "Mapa cargado" sin dibujar nada); `clearAdminMapOverlay()` es defensivo si aún no hay mapa; `closePanel()` limpia también `adminMapOverlay`.
  - **Mapa base:** CARTO → **Esri Canvas** (`server.arcgisonline.com/.../Canvas/World_Dark_Gray_Base` y `World_Light_Gray_Base`), gratuito y sin clave, en el mapa principal y en el admin. Añadido `server.arcgisonline.com` a `TILE_PATTERNS` del Service Worker.
  - **Service Worker:** el timeout de tiles sube de 1500 ms a 4000 ms (demasiado agresivo en campo) y el respaldo de un tile fallido pasa de un `204 No Content` — que hacía que `<img>` disparara `error` y Leaflet dejara el hueco en blanco para siempre — a un **PNG 1x1 transparente real**.
- **Archivos y funciones:** `js/admin-manager.js` (`openPanel`, `closePanel`, `fetchTable`, `renderMapSelector`, `loadAdminMap`, `loadAdminPDF`, `clearAdminMapOverlay`, `renderAdminMarkers`, `renderMapTab`, nuevo `refreshAdminMapSize`), `js/app.js` (`initMap`: `darkTiles`/`lightTiles`), `sw.js` (`TILE_PATTERNS`, `staleWhileRevalidate`, `TRANSPARENT_PNG`).
- **Bumps de versión:** `2.10.10` → `2.10.11`.
